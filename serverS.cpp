...
int main(int argc, const char * argv[]) {
    ...
    server_addr_udp_main_server.sin_port=htons(41495);
    ...
    printf("Server S is up and running using UDP on port 41495\n");
    ...
    while(true){
        ...
        if(ans==1){
            ...
            printf("Server S finished sending the availability status of code %s to the Main Server using UDP on port 41495\n",recv_buffer);
            //rewrite_file();
        }else if(ans==0){
        ...
        if(!admin_flag){
            ...
            printf("Server S finished sending the inventory status to the Main server using UDP on port 41495\n");
        }
    }
    return 0;
}
