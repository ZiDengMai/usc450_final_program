...
void rewrite_file(){
    ...
    outfile.open("./history.txt"); // 默认以写入的方式打开文件
    char out[1024];
    char temp[1024];
    string t="";
    //cout<<an<<endl;
...
int main(int argc, const char * argv[]) {
    ...
    server_addr_udp_main_server.sin_port=htons(43495);
    ...
    printf("Server H is up and running using UDP on port 43495\n");
    ...
    while(true){
        ...
        if(!admin_flag){
            printf("Server H finished sending the availability status of code %s to the Main Server using UDP on port 43495\n",recv_buffer);
        }else{
        ...
        if(!admin_flag){
            ...
            printf("Server H finished sending the inventory status to the Main server using UDP on port 43495\n");
        }
    }
    return 0;
}
